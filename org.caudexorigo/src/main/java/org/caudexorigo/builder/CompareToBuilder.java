 ...
 * 
 * It is consistent with <code>equals(Object)</code> and <code>hashcode()</code> built with {@link EqualsBuilder} and {@link HashCodeBuilder}.</p>
 * 
 * <p>
 * Two Objects that compare equal using <code>equals(Object)</code> should normally also compare equal using <code>compareTo(Object)</code>.
 * </p>
 * 
 * <p>
 * All relevant fields should be included in the calculation of the comparison. Derived fields may be ignored. The same fields, in the same order, should be used in both <code>compareTo(Object)</code> and <code>equals(Object)</code>.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * A typical implementation of <code>compareTo(Object)</code> using <code>reflectionCompare</code> looks like:
 * </p>
 * 
 * <pre>
 * public int compareTo(Object o)
...
public class CompareToBuilder
{
	 ...
	 * 
	 * <p>
	 * Fields can be private, thus <code>AccessibleObject.setAccessible</code> is used to bypass normal access control checks. This will fail under a security manager unless the appropriate permissions are set.
	 * </p>
	 * 
	 * <ul>
	 ...
	 * 
	 * <p>
	 * If both <code>lhs</code> and <code>rhs</code> are <code>null</code>, they are considered equal.
	 * </p>
	 * 
	 * @param lhs
	 *            left-hand object
	 ...
	 * <ul>
	 * <li>Static fields will not be compared</li>
	 * <li>If <code>compareTransients</code> is <code>true</code>, compares transient members. Otherwise ignores them, as they are likely derived fields.</li>
	 * <li>Superclass fields will be compared</li>
	 * </ul>
	 * 
	 * <p>
	 ...
	 * @return a negative integer, zero, or a positive integer as <code>lhs</code> is less than, equal to, or greater than <code>rhs</code>
	 * @throws NullPointerException
	 *             if either <code>lhs</code> or <code>rhs</code> (but not both) is <code>null</code>
	 * @throws ClassCastException
	 *             if <code>rhs</code> is not assignment-compatible with <code>lhs</code>
	 */
	 ...
	 * <ul>
	 * <li>Static fields will not be compared</li>
	 * <li>If the <code>compareTransients</code> is <code>true</code>, compares transient members. Otherwise ignores them, as they are likely derived fields.</li>
	 * <li>Compares superclass fields up to and including <code>reflectUpToClass</code>. If <code>reflectUpToClass</code> is <code>null</code>, compares all superclass fields.</li>
	 * </ul>
	 * 
	 * <p>
	 * If both <code>lhs</code> and <code>rhs</code> are <code>null</code>, they are considered equal.
	...
	/**
	 * <p>
	 * Appends to <code>builder</code> the comparison of <code>lhs</code> to <code>rhs</code> using the fields defined in <code>clazz</code>.
	 * </p>
	 * 
	 * @param lhs
	 *            left-hand object
	...
	/**
	 * <p>
	 * Appends to the <code>builder</code> the <code>compareTo(Object)</code> result of the superclass.
	 * </p>
	 * 
	 ...
	 * <li>Check if either <code>lhs</code> or <code>rhs</code> is <code>null</code>, a <code>null</code> object is less than a non-<code>null</code> object</li>
	 ...
	 * 
	 * <p>
	 * If <code>lhs</code> is an array, array comparison methods will be used. Otherwise <code>comparator</code> will be used to compare the objects. If <code>comparator</code> is <code>null</code>, <code>lhs</code> must implement {@link Comparable} instead.
	 * </p>
	 * 
	 * @param lhs
	 *            left-hand object
	 ...
	 *            right-hand object
	 * @param comparator
	 *            <code>Comparator</code> used to compare the objects, <code>null</code> means treat lhs as <code>Comparable</code>
	 * @return this - used to chain append calls
	 * @throws ClassCastException
	 *             if <code>rhs</code> is not assignment-compatible with <code>lhs</code>
	 * @since 2.0
	 ...
	 *            right-hand array
	 * @param comparator
	 *            <code>Comparator</code> to use to compare the array elements, <code>null</code> means to treat <code>lhs</code> elements as <code>Comparable</code>.
	 * @return this - used to chain append calls
	 * @throws ClassCastException
	 *             if <code>rhs</code> is not assignment-compatible with <code>lhs</code>
	 * @since 2.0
