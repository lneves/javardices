...
public class JdkHttpHandlerContainer implements HttpHandler, Container
{
	 ...
	 * 
	 * @param appHandler
	 *            Jersey application handler for which the container should be initialized.
	 */
	...
	public void handle(final HttpExchange exchange) throws IOException
	{
		...
		/*
		 * The following is madness, there is no easy way to get the complete URI of the HTTP request!!
		 * 
		...
		ContainerRequest requestContext = new ContainerRequest(baseUri, requestUri, exchange.getRequestMethod(), getSecurityContext(exchange.getPrincipal(), isSecure), new MapPropertiesDelegate());
	...
	private final static class ResponseWriter implements ContainerResponseWriter
	{
		 ...
		 * 
		 * @param exchange
		 *            Exchange of the {@link HttpServer JDK Http Server}
		 */
		...
		public OutputStream writeResponseStatusAndHeaders(long contentLength, ContainerResponse context) throws ContainerException
		{
			...
			try
			{
				...
				{
					exchange.sendResponseHeaders(context.getStatus(), getResponseLength(contentLength));
				}
			}
		 ...
		 * 
		 * This method should be called by the container at the end of the handle method to make sure that the ResponseWriter was committed.
		 */
