...
public class UnsynchronizedBufferedReader extends Reader {
  ...
  /**
   * Reads characters into a portion of an array, reading from the underlying stream if
   * necessary.
   */
  private int read1(char[] cbuf, int off, int len) throws IOException {
    if (nextChar >= nChars) {
       ...
       * mark/reset activity, and if line feeds are not being skipped, do not bother to
       * copy the characters into the local buffer. In this way buffered streams will
       * cascade harmlessly.
       */
   ...
   * <code>{@link Reader}</code> class. As an additional convenience, it attempts to read
   * as many characters as possible by repeatedly invoking the <code>read</code> method of
   * the underlying stream. This iterated <code>read</code> continues until one of the
   ...
   * indicate end-of-file then this method returns <code>-1</code>. Otherwise this method
   * returns the number of characters actually read.
   * 
   * <p>
   * Subclasses of this class are encouraged, but not required, to attempt to read as many
   * characters as possible in the same fashion.
   * 
   * <p>
   * Ordinarily this method takes characters from this stream's character buffer, filling
   * it from the underlying stream as necessary. If, however, the buffer is empty, the
   * mark is not valid, and the requested length is at least as large as the buffer, then
   * this method will read characters directly from the underlying stream into the given
  ...
  /**
   * Tells whether this stream is ready to be read. A buffered character stream is ready
   * if the buffer is not empty, or if the underlying character stream is ready.
   * 
   * @exception IOException If an I/O error occurs
   */
   ...
   *        preserving the mark. An attempt to reset the stream after reading characters
   *        up to this limit or beyond may fail. A limit value larger than the size of the
   *        input buffer will cause a new buffer to be allocated whose size is no smaller
   ...
   * 
   * @exception IOException If the stream has never been marked, or if the mark has been
   *            invalidated
   */
