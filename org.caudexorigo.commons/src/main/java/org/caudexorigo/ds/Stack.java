...
/**
 * Convenient implementation of a Last In First Out (LIFO) stack. This implementation
 * differs from the one in java.util.Stack in two ways.
 * 
 * First, like most of the collection APIs, it is unsynchronized for better performance
 * when synchronization is not required. If a synchronized stack is required, you can use
 * the {@link java.util.Collections#synchronizedCollection(java.util.Collection)
 * Collections.synchronizedCollection()} method to retrieve a synchronized instance.
 * 
 * Second, it does not expose its internal implementation via its superclass. Extending
 * <tt>AbstractCollection</tt> instead of <tt>Vector</tt> allows objects of this class to
 * be used interchangably with other collection framework classes without exposing its
...
public final class Stack<E> extends AbstractCollection<E> {
   ...
   * The items will be pushed on to the stack in their order in the collection.
   * 
  ...
  /**
   * Gets an iterator for elements on the stack. This iterator starts at the bottom of the
   * stack and proceeds to the top.
   * 
  ...
  /**
   * Gets an iterator for elements on the stack. The iterator starts at the top of the
   * stack and proceeds to the bottom of the stack.
   * 
   * @return an iterator over stack elements.
   */
  ...
  /**
   * Iterator that traverses a list in reverse order. It does this by just adapting the
   * ListIterator of the list.
   */
