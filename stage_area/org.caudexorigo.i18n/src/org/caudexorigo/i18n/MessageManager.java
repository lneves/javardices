...
/**
 * The <code>MessageManager</code> provides methods for retrieving localized messages and adding custom message providers. This class should not be called directly for other purposes than registering a custom {@link MessageProvider} or retrieving information about available message entries.
 * <p>
 * To access localized messages a subclass of the {@link LocalizedBundle} class such as <code>LocalizedText </code> should be used:
 * 
 * <pre>
 ...
 * 
 * <p>
 * You can call {@link MessageManager#getText(String,String,Object[],Locale) getText} directly, but if you do so, you have to ensure that the given entry key really exists and to deal with the {@link MessageNotFoundException} exception that will be thrown if you try to access a not existing entry.
 * </p>
 * 
 */
public class MessageManager
{
	private static Map<String, MessageProvider> messageProviders = new LinkedHashMap<String, MessageProvider>();
	 ...
	 * 
	 * @param providerId
	 *            Id of the provider used for uninstallation and qualified naming.
	...
	/**
	 * Remove all <code>{@link MessageProvider}</code>s from the <code>MessageManager</code>. Used for tearing down unit tests.
	 */
	...
	public static boolean exists(String providerId)
	{
		return messageProviders.containsKey(providerId);
	}
	 ...
	 * 
	 * @param id
	 *            The identifier that will be used to retrieve the message bundle
	 * @param entry
	 *            The desired message entry
	 * @param arguments
	 *            The dynamic parts of the message that will be evaluated using the standard java text formatting abilities.
	 * @param locale
	 *            The locale in which the message will be printed
	 * @exception MessageNotFoundException
	 *                Will be thrown if no message bundle can be found for the given id or if the desired message entry is missing in the retrieved bundle
	...
	public static String getText(String id, String entry, Object[] arguments, Locale locale) throws MessageNotFoundException
	{
		...
		{
			String text = i.next().getText(id, entry, locale);
			if (text != null)
				return (arguments != null && arguments.length > 0) ? MessageFormat.format(text, arguments) : text;
		}
		throw new MessageNotFoundException(MessageFormat.format(I18nUtils.INTERNAL_MESSAGES.getString(I18nUtils.MESSAGE_ENTRY_NOT_FOUND), new Object[] { id, entry }));
	}
	...
	public static String getText(String id, String entry, Object[] arguments, Locale locale, String defaultText)
	{
		...
		{
			return (arguments != null && arguments.length > 0) ? MessageFormat.format(defaultText, arguments) : defaultText;
		}
	}
	 ...
	 *            The locale in which the message will be printed
	 * @exception MessageNotFoundException
	 *                Will be thrown if the requested message provider cannot be found or no message bundle can be found for the given id or if the desired message entry is missing in the retrieved bundle
	 * @return The localized text
	 */
	 ...
	 */
	public static Map<String, String> getEntries(String id, Locale locale) throws MessageNotFoundException
	{
		if (messageProviders.isEmpty())
			throw new MessageNotFoundException("No MessageProvider registered");
	 ...
	 * Returns a map containing all available message entries for the given locale. The map contains keys of type {@link String}containing the keys of the available message entries and values of type {@link String} containing the localized message entries.
	 */
	public static Map<String, String> getEntries(String providerId, String id, Locale locale) throws MessageNotFoundException
	{
		MessageProvider provider = messageProviders.get(providerId);
		if (provider == null)
			throw new MessageNotFoundException("Provider '" + providerId + "' not installed");
