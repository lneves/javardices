...
/**
 * <p>
 * <code>Verifier</code> checks names and data for compliance with XML 1.0 and Namespaces
 * in XML rules.
 * </p>
 * 
...
final class Verifier {
  ...
  private static void loadFlags(ClassLoader loader) {
    ...
    try {
      InputStream raw = loader.getResourceAsStream("org/caudexorigo/nu/xom/characters.dat");
      if (raw == null) {
        throw new RuntimeException(
            "Broken XOM installation: could not load org/caudexorigo/nu/xom/characters.dat");
      }
      // buffer this????
  ...
  /**
   * <p>
   * Check whether <code>name</code> is a non-colonized name as defined in
   * <cite>Namespaces in XML</cite>.
   * </p>
   * 
   * @param name <code>String</code> name to check
   * 
   * @throws IllegalNameException if <code>name</code> is not a non-colonized name
   */
  ...
  /**
   * <p>
   * This methods checks whether a string contains only characters allowed by the XML 1.0
   * specification.
   * </p>
   *
   * @param text <code>String</code> value to verify
   * 
   * @throws IllegalCharacterDataException if <code>text</code> is not legal PCDATA
   */
      ...
      if (result >= 0xD800 && result <= 0xDBFF) {
        try {
          ...
          if (low < 0xDC00 || low > 0xDFFF) {
            IllegalCharacterDataException ex = new IllegalCharacterDataException(
                "Bad surrogate pair");
            ex.setData(text);
            throw ex;
          }
  ...
  /**
   * <p>
   * Checks a string to see if it is a syntactically correct RFC 3986 URI reference. Both
   * absolute and relative URIs are supported, as are URIs with fragment identifiers.
   * </p>
   * 
   * @param uri <code>String</code> containing the potential URI
   * 
   * @throws MalformedURIException if this is not a legal URI reference
   */
  static void checkURIReference(String uri) {
    ...
    try {
      ...
      if (parsed.authority != null)
        checkAuthority(parsed.authority);
      checkPath(parsed.path);
      if (parsed.fragment != null)
        checkFragment(parsed.fragment);
      if (parsed.query != null)
        checkQuery(parsed.query);
  ...
  private static boolean isQueryCharacter(char c) {
    ...
    switch (c) {
      ...
      case '$':
        return true;
      case '%':
        return false; // tested in checkQuery
      case '&':
        return true;
      case '\'':
        return true;
  ...
  /**
   * <p>
   * Checks a string to see if it is an RFC 3986 absolute URI reference. URI references
   * can contain fragment identifiers. Absolute URI references must have a scheme.
   * </p>
   * 
  ...
  static boolean isSchemeCharacter(char c) {
    ...
    /*
     * The : and the ? cannot be reached here because they'll have been parsed out
     * separately before this method is called. They're included here strictly for
     * alignment so the compiler will generate a table lookup.
     */
    switch (c) {
      case '+':
        return true;
  ...
  /**
   * Check to see that this string is an absolute URI, neither a relative URI nor a URI
   * reference.
   * 
   */
