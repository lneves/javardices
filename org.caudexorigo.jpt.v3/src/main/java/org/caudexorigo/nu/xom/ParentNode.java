...
public abstract class ParentNode extends Node
{
	...
	/**
	 * <p>
	 * Creates a new <code>ParentNode</code> object. Can only be invoked by other members of the <code>org.caudexorigo.nu.xom</code> package.
	 * </p>
	 * 
	 */
	ParentNode()
	...
	/**
	 * <p>
	 * Inserts a child node at the specified position. The child node previously at that position (if any) and all subsequent child nodes are moved up by one. That is, when inserting a node at 2, the old node at 2 is moved to 3, the old child at 3 is moved to 4, and so forth. Inserting at position 0 makes the child the first child of this node. Inserting at the position <code>getChildCount()</code> makes the child the last child of the node.
	 * </p>
	 * 
	 * <p>
	 ...
	 *             if this node cannot have a child of the argument's type
	 * @throws MultipleParentException
	 *             if <code>child</code> already has a parent
	 * @throws NullPointerException
	 *             if <code>child</code> is null
	 * @throws IndexOutOfBoundsException
	 *             if the position is negative or greater than the number of children of this node
	 */
	 ...
	 * 
	 * @throws IndexOutOfBoundsException
	 *             if the index is negative or greater than or equal to the number of children of this node
	 */
	...
	/**
	 * <p>
	 * Returns the position of a node within the children of this node. This is a number between 0 and one less than the number of children of this node. It returns -1 if <code>child</code> does not have this node as a parent.
	 * </p>
	 * 
	 * <p>
	...
	public int indexOf(Node child)
	{
		...
		/*
		 * lastPosition++; if (lastPosition != children.size()) { if (child == children.get(lastPosition)) { return lastPosition; } else lastPosition = -1; } lastPosition = children.indexOf(child); return lastPosition;
		 */
	 ...
	 *             if <code>child</code> is not in fact a child of this node
	 */
	...
	/**
	 * <p>
	 * Replaces an existing child with a new child node. If <code>oldChild</code> is not a child of this node, then a <code>NoSuchChildException</code> is thrown.
	 * </p>
	 * 
	 * @param oldChild
	 ...
	 * 
	 * @throws MultipleParentException
	 *             if <code>newChild</code> already has a parent
	 * @throws NoSuchChildException
	 *             if <code>oldChild</code> is not a child of this node
	 * @throws NullPointerException
	 *             if either argument is null
	 * @throws IllegalAddException
	 *             if this node cannot have children of the type of <code>newChild</code>
	 */
	 ...
	 * 
	 * <p>
	 * You can also add an <code>xml:base</code> attribute to an element in the same way you'd add any other namespaced attribute to an element. If an element's base URI conflicts with its <code>xml:base</code> attribute, then the value found in the <code>xml:base</code> attribute is used.
	 * </p>
	 * 
	 * <p>
	 * If the base URI is null or the empty string and there is no <code>xml:base</code> attribute, then the base URI is determined by the nearest ancestor node which does have a base URI. Moving such a node from one location to another can change its base URI.
	 * </p>
	 * 
	 * @param URI
