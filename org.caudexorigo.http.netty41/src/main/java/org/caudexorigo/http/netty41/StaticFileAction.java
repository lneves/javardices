...
public class StaticFileAction extends HttpAction
{
	...
	public void service(ChannelHandlerContext ctx, FullHttpRequest request, FullHttpResponse rsp)
	{
		...
		if (cacheDuration > 0)
		{
			response.headers().set(HttpHeaderNames.CACHE_CONTROL, String.format("max-age=%s", cacheDuration));
			response.headers().set(HttpHeaderNames.LAST_MODIFIED, HttpDateFormat.getHttpDate(new Date(file.lastModified())));
