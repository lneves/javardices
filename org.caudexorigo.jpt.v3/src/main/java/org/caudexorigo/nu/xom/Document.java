...
/**
 * <p>
 * The <code>Document</code> class represents a complete XML document including its root element, prolog, and epilog.
 * </p>
 * 
...
public class Document extends ParentNode
{
	 ...
	 *             if <code>root</code> already has a parent
	 */
	 ...
	 * 
	 * @throws MultipleParentException
	 *             if <code>doctype</code> belongs to another document
	 * @throws NullPointerException
	 *             if <code>doctype</code> is null
	 * 
	 */
	 ...
	 *             if <code>URI</code> is not a legal absolute URI
	 */
	...
	/**
	 * <p>
	 * Removes the child of this document at the specified position. Indexes begin at 0 and count up to one less than the number of children of this document. The root element cannot be removed. Instead, use <code>setRootElement</code> to replace the existing root element with a different element.
	 * </p>
	 * 
	 * @param position
	 ...
	 *             if the index is negative or greater than the number of children of this document - 1
	...
	/**
	 * <p>
	 * Removes the specified child from this document. The root element cannot be removed. Instead, use <code>setRootElement</code> to replace the existing root element with a different element.
	 * </p>
	 * 
	 * @param child
	 *            node to remove
	...
	/**
	 * <p>
	 * Replaces an existing child with a new child node. If <code>oldChild</code> is not a child of this node, then a <code>NoSuchChildException</code> is thrown. The root element can only be replaced by another element.
	 * </p>
	 * 
	 * @param oldChild
	 ...
	 *             if <code>newChild</code> is an attribute or a text node
	 * @throws WellformednessException
	 *             if <code>newChild</code> <code>oldChild</code> is an element and <code>newChild</code> is not
	 */
	...
	/**
	 * <p>
	 * Returns the actual complete, well-formed XML document as a <code>String</code>. Significant white space is preserved. Insignificant white space in tags, the prolog, the epilog, and the internal DTD subset is not preserved. Entity and character references are not preserved. The entire document is contained in this one string.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Returns a string representation of this document suitable for debugging and diagnosis. This is <em>not</em> the XML representation of this document.
	 * </p>
	 * 
