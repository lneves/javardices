...
public class ParameterDecoder
{
	...
	private void initParameters()
	{
		if (parameters == null)
		{
			...
			// parameters = (new QueryStringDecoder(request.uri(), charset, true)).getParameters();

			String content_type = request.headers().get(HttpHeaderNames.CONTENT_TYPE);

			if (request.method().equals(HttpMethod.POST) && (StringUtils.contains(content_type, X_WWW_FORM_URLENCODED)) && (HttpUtil.getContentLength(request) > 0))
			{
