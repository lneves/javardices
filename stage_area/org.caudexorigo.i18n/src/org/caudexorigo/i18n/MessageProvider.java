...
/**
 * The <code>MessageProvider</code> interface specifies the methods that must be implemented by each message provider in order to be pluggable into the <code>MessageManager</code>.
 * 
 */
public interface MessageProvider
{
	/**
	 * @param id
	 *            unique id that specifies a particular message
	 * @param entry
	 *            specifies a particular entry in the specified message
	 * @param locale
	 *            the locale for which this text should be provided
	 * @return returns the localized message entry matching the given message id, entry key and locale. If no match is found for the given locale, the parent locale is used, and finally the default. If the id is found but the entry is not, null is returned.
