...
class XOMHandler implements ContentHandler, LexicalHandler, DeclHandler, DTDHandler {
  ...
  // See
  // http://www.servlets.com/archive/servlet/ReadMsg?msgId=554071&listName=jdom-interest
  ...
  public void characters(char[] text, int start, int length) {
    ...
    if (length <= 0)
      return;
    if (textString == null)
      textString = new String(text, start, length);
    else {
      if (buffer == null)
        buffer = new StringBuffer(textString);
  ...
  private void flushText() {
    ...
    if (textString != null) {
      ...
      if (!inCDATA) {
        ...
        result = factory.makeText("<![CDATA[" + textString + "]]>"); // factory.makeCDATASection(textString);
      }
      for (int i = 0; i < result.size(); i++) {
        Node node = result.get(i);
        if (node.isAttribute()) {
    ...
    for (int i = 0; i < result.size(); i++) {
      ...
      if (!inDTD) {
        ...
        if (node.isProcessingInstruction() || node.isComment()) {
          ...
          throw new XMLException("Factory tried to put a " + node.getClass().getName()
  ...
  protected boolean inInternalSubset() {
    ...
    if (currentURI == null)
      return false;
    if (currentURI.equals(this.documentBaseURI))
      return true;
    return false;
  }
