...
public class NettyHttpServer {
  ...
  public synchronized void start() {
    ...
    try {
      ...
      NettyHttpServerInitializer server_init = new NettyHttpServerInitializer(_mapper,
          getRequestObserver(), getResponseFormtter(), getMaxContentLength(), getValidateHeaders());
  ...
  public synchronized void startSsl(final HttpSslContext http_ssl_ctx) {
    ...
    try {
      HttpProtocolHandler http_handler = new HttpProtocolHandler(_mapper, getRequestObserver(),
          getResponseFormtter());
      NettySslHttpServerInitializer server_init = new NettySslHttpServerInitializer(sslContext,
          http_handler);
      ServerBootstrap b = new ServerBootstrap();
      setupBootStrap(b);
