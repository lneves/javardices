 ...
 * <li>A list of namespace declarations for this element (not including those inherited from its parent)</li>
...
public class Element extends ParentNode
{
	 ...
	 *             if <code>name</code> is not a legal XML 1.0 non-colonized name
	 */
	 ...
	 * 
	 * @throws IllegalNameException
	 *             if <code>name</code> is not a legal XML 1.0 name
	 * @throws NamespaceConflictException
	 *             if <code>name</code>'s prefix cannot be used with <code>uri</code>
	 * @throws MalformedURIException
	 *             if <code>uri</code> is not an RFC 3986 absolute URI reference
	 */
	 ...
	 * 
	 * @return a comatose list containing the child elements of this element with the specified name
	 */
	 ...
	 *            the name of the elements included in the list
	 * @param namespaceURI
	 *            the namespace URI of the elements included in the list
	 * 
	 * @return a comatose list containing the child elements of this element with the specified name in the specified namespace
	 */
	 ...
	 * 
	 * @return the first child element with the specified local name in no namespace or null if there is no such element
	 */
	 ...
	 * 
	 * @return the first child with the specified local name in the specified namespace, or null if there is no such element
	 */
	 ...
	 *             if the attribute is already attached to an element
	 * @throws NamespaceConflictException
	 ...
	 * 
	 * <p>
	 * Returns the number of attributes of this <code>Element</code>, not counting namespace declarations. This is always a non-negative number.
	 * </p>
	 * 
	 ...
	 *             if the index is negative or greater than or equal to the number of attributes of this element
	 * 
	 */
	 ...
	 *            the namespace of the attribute
	 * 
	 * @return the value of the attribute of this element with the specified name and namespace
	 */
	 ...
	 * 
	 * @throws IllegalNameException
	 *             if <code>localName</code> is not a legal, non-colonized name
	 */
	 ...
	 * 
	 * @throws MalformedURIException
	 *             if <code>uri</code> is not an absolute RFC 3986 URI reference
	 * @throws NamespaceException
	 *             if this element has a prefix and <code>uri</code> is null or the empty string; or if the element's prefix is shared by an attribute or additional namespace
	 */
	 ...
	 * 
	 * @throws IllegalNameException
	 *             if <code>prefix</code> is not a legal XML non-colonized name
	 * @throws NamespaceConflictException
	 *             if <code>prefix</code> is already in use by an attribute or additional namespace with a different URI than the element itself
	 */
	...
	/**
	 * <p>
	 * Inserts a child node at the specified position. Inserting at position 0 makes the child the first child of this node. Inserting at the position <code>getChildCount()</code> makes the child the last child of the node.
	 * </p>
	 * 
	 * <p>
	 ...
	 *             if <code>child</code> is a <code>Document</code>
	 * @throws MultipleParentException
	 *             if <code>child</code> already has a parent
	 * @throws NullPointerException
	 *             if <code>child</code> is null
	 * @throws IndexOutOfBoundsException
	 *             if the position is negative or greater than the number of children of this element.
	 */
	 ...
	 * 
	 * @throws IllegalAddException
	 *             if this node cannot have children of this type
	 * @throws NullPointerException
	 ...
	 * 
	 * <p>
	 * Subclassers should note that the default implementation of this method does <strong>not</strong> call <code>removeChild</code> or <code>detach</code>. If you override <code>removeChild</code>, you'll probably need to override this method as well.
	 * </p>
	 * 
	 ...
	 * 
	 * @throws MalformedURIException
	 *             if <code>URI</code> is not an RFC 3986 URI reference
	 * @throws IllegalNameException
	 *             if <code>prefix</code> is not a legal XML non-colonized name or the empty string
	 * @throws NamespaceConflictException
	...
	/**
	 * <p>
	 * Removes the mapping of the specified prefix. This method only removes additional namespaces added with <code>addNamespaceDeclaration</code>. It has no effect on namespaces of elements and attributes. If the prefix is not used on this element, this method does nothing.
	 * </p>
	 * 
	 * @param prefix
	...
	/**
	 * <p>
	 * Returns the number of namespace declarations on this element. This counts the namespace of the element itself (which may be the empty string), the namespace of each attribute, and each namespace added by <code>addNamespaceDeclaration</code>. However, prefixes used multiple times are only counted once; and the <code>xml</code> prefix used for <code>xml:base</code>, <code>xml:lang</code>, and <code>xml:space</code> is not counted even if one of these attributes is present on the element.
	 * </p>
	 * 
	 * <p>
	 * The return value is almost always positive. It can be zero if and only if the element itself has the prefix <code>xml</code>; e.g. <code>&lt;xml:space /></code>. This is not endorsed by the XML specification. The prefix <code>xml</code> is reserved for use by the W3C, which has only used it for attributes to date. You really shouldn't do this. Nonetheless, this is not malformed so XOM allows it.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Returns the index<sup>th</sup> namespace prefix <em>declared</em> on this element. Namespaces inherited from ancestors are not included. The index is purely for convenience, and has no meaning in itself. This includes the namespaces of the element name and of all attributes' names (except for those with the prefix <code>xml</code> such as <code>xml:space</code>) as well as additional declarations made for attribute values and element content. However, prefixes used multiple times (e.g. on several attribute values) are only reported once. The default namespace is reported with an empty string prefix if present. Like most lists in Java, the first prefix is at index 0.
	 * </p>
	 * 
	 * <p>
	 ...
	 * 
	 * @throws IndexOutOfBoundsException
	 *             if <code>index</code> is negative or greater than or equal to the number of namespaces declared by this element.
	 * 
	 */
	 ...
	 * 
	 * <p>
	 * Sets the URI from which this element was loaded, and against which relative URLs in this element will be resolved, unless an <code>xml:base</code> attribute overrides this. Setting the base URI to null or the empty string removes any existing base URI.
	 * </p>
	 * 
	 * @param URI
	 *            the new base URI for this element
	 * 
	 * @throws MalformedURIException
	 *             if <code>URI</code> is not a legal RFC 3986 absolute URI
	 */
	 ...
	 * 
	 * <p>
	 * Returns the absolute base URI against which relative URIs in this element should be resolved. <code>xml:base</code> attributes <em>in the same entity</em> take precedence over the actual base URI of the document where the element was found or which was set by <code>setBaseURI</code>.
	 * </p>
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * If the element's <code>xml:base</code> attribute contains a value that is a syntactically illegal URI (e.g. %GF.html"), then the base URI is application dependent. XOM's choice is to behave as if the element did not have an <code>xml:base</code> attribute.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Creates a very shallow copy of the element with the same name and namespace URI, but no children, attributes, base URI, or namespace declaration. This method is invoked as necessary by the {@link org.caudexorigo.nu.xom.Element#copy() copy} method and the {@link org.caudexorigo.nu.xom.Element#Element(org.caudexorigo.nu.xom.Element) copy constructor}.
	 * </p>
	 * 
	 * <p>
	...
	/**
	 * <p>
	 * Returns a string representation of this element suitable for debugging and diagnosis. This is <em>not</em> the XML representation of the element.
	 * </p>
	 * 
