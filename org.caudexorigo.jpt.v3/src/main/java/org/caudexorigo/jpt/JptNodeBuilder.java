...
public class JptNodeBuilder extends BaseJptNodeBuilder {
  ...
  private static final String APPEND_SLOT_NS =
      "http://xml.zope.org/namespaces/metal/append-to-slot";
  ...
  private Map<String, String> extractMacroParams(String uri_qs) {
    try {
      ...
      for (String pair : pairs) {
        ...
        query_pairs.put(URLDecoder.decode(pair.substring(0, idx), "UTF-8"), URLDecoder.decode(pair
            .substring(idx + 1), "UTF-8"));
      }
      return query_pairs;
    } catch (Throwable t) {
  ...
  private void prepareMacro(URI muri, Document macroDocument, Node useMacroNode, String macroPath,
      Map<String, String> macroParams) throws ValidityException, ParsingException, IOException {
    ...
    if (subtree == null) {
      throw new IllegalArgumentException(String.format("could not find macro '%s' in template '%s'",
          macroName, macroPath));
    }
  ...
  private void processTalAttributes(Element el) {
    ...
    if (attribute != null) {
      ...
      for (int l = aattribute1.length; k < l; k++) {
        ...
        JptAttributeNode jattr = new JptAttributeNode(tal_attribute, _isInSlot, _jptConf
            .escapeOutput());
  ...
  private void processTalConditionalAttributes(Element el) {
    ...
    if (attribute != null) {
      ...
      for (int l = aattribute1.length; k < l; k++) {
        ...
        JptConditionalAttributeNode jattr = new JptConditionalAttributeNode(tal_attribute,
            _isInSlot);
        ((JptParentNode) pnodes.peek()).appendChild(jattr);
      }
    }
  ...
  private void processTalCondition(Element el) {
    ...
    if (attribute != null) {
      ...
      // System.out.println("JptNodeBuilder.processTalCondition.expression: " +
  ...
  private void processTalOmitTag(Element el) {
    ...
    if (omit_tag_attr != null) {
      // System.out.println("JptNodeBuilder.processTalOmitTag.parent.begin: " +
      // (((JptParentNode) pnodes.peek())).getClass().getCanonicalName());
      ...
      String omit_tag_condition = StringUtils.isBlank(omit_tag_attr.getValue()) ? "false"
          : String.format("!%s", omit_tag_attr.getValue());// conditional nodes output on
                                                           // true, revert the logic,
                                                           // should not output on true
      ...
      // System.out.println("JptNodeBuilder.processTalOmitTag.parent.end: " +
      // (((JptParentNode) pnodes.peek())).getClass().getCanonicalName());
    } else {
      process(el);
    }
  ...
  private void processTalContent(Element el) {
    ...
    if (attribute != null) {
      ...
      if (omit_tag_attr != null) {
        ...
        String omit_tag_condition = StringUtils.isBlank(omit_tag_attr.getValue()) ? "false"
            : String.format("!%s", omit_tag_attr.getValue());// conditional nodes output
                                                             // on true, revert the logic,
                                                             // should not output on true
      ...
      if (el.getBaseURI().equals(APPEND_SLOT_NS)) {
        ...
        jout = new JptOutputExpressionNode(attribute.getValue(), _isInSlot, _jptConf
            .escapeOutput());
      }
  ...
  private void processMetalUseMacro(Element el) throws ValidityException, ParsingException,
      IOException {
  ...
  private void processMetalDefineSlot(Element el) throws ValidityException, ParsingException,
      IOException {
