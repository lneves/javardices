...
public class JptOutputExpressionNode extends JptNode {
  private static final String TEXT = "text "; // DEFAULT - "angle-brackets and ampersands
                                              // with their HTML entity equivalents"
  private static final String STRUCTURE = "structure "; // "passes the replacement text
                                                        // through unchanged";
  ...
  public void render(Map<String, Object> context, Writer out) throws IOException {
    ...
    try {
      ...
      String emsg = String.format(
          "Error processing JptOutputExpressionNode:%nexpression: '%s';%ncontext: %s;%nmessage: '%s'",
