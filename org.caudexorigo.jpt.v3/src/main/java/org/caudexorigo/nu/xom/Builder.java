...
/**
 * <p>
 * This class is responsible for creating XOM <code>Document</code> objects from a URL, file, string, or input stream by reading an XML document. A SAX parser is used to read the document and report any well-formedness errors.
 * </p>
 * 
...
public class Builder
{
	...
	/**
	 * <p>
	 * Creates a <code>Builder</code> that uses the default node factory and chooses among any available SAX2 parsers. In order of preference, it looks for:
	 * </p>
	 * 
	 * <ol>
	 ...
	 * <li>The platform default specified by the <code>org.xml.sax.driver</code> system property</li>
	 * </ol>
	 * 
	 * <p>
	...
	/**
	 * <p>
	 * Creates a <code>Builder</code> based on an optionally validating parser. If the <code>validate</code> argument is true, then a validity error while parsing will cause a fatal error; that is, it will throw a <code>ValidityException</code>.
	 * </p>
	 * 
	 * @param validate
	...
	/**
	 * <p>
	 * Creates a <code>Builder</code> based on an optionally validating parser that builds node objects with the supplied factory. If the <code>validate</code> argument is true, then a validity error while parsing will cause a fatal error; that is, it will throw a <code>ValidityException</code>.
	 * </p>
	 * 
	 * @param validate
	 *            true if the parser should validate the document while parsing
	 * @param factory
	 *            the <code>NodeFactory</code> that creates the node objects for this <code>Builder</code>
	 *
	 * @throws XMLException
	 *             if no satisfactory parser is installed in the local class path
	 */
	...
	/**
	 * <p>
	 * Creates a <code>Builder</code> that uses the specified SAX <code>XMLReader</code>. Custom SAX features and properties such as schema validation can be set on this <code>XMLReader</code> before passing it to this method.
	 * </p>
	 * 
	 * @param parser
	 *            the SAX2 <code>XMLReader</code> that parses the document
	 * 
	 * @throws XMLException
	 *             if <code>parser</code> does not support the features XOM requires
	 */
	...
	/**
	 * <p>
	 * Creates a <code>Builder</code> that uses the specified <code>NodeFactory</code> to create node objects.
	 * </p>
	 * 
	 * @param factory
	 *            the <code>NodeFactory</code> that creates the node objects for this <code>Builder</code>
	...
	/**
	 * <p>
	 * Creates a optionally validating <code>Builder</code> based on the specified parser object. Custom SAX features and properties such as schema validation can be set on this <code>XMLReader</code> before passing it to this method.
	 * </p>
	 * 
	 * <p>
	 * If the validate argument is true, then a validity error while parsing will cause a fatal error; that is, it will throw a <code>ParsingException</code>
	 * </p>
	 * 
	 * @param parser
	 *            the SAX2 <code>XMLReader</code> that parses the document
	...
	/**
	 * <p>
	 * Creates an optionally validating <code>Builder</code> that reads data from the specified parser object and constructs new nodes using the specified factory object. Custom SAX features and properties such as schema validation can be set on this <code>XMLReader</code> before passing it to this method.
	 * </p>
	 * 
	 * <p>
	 * If the <code>validate</code> argument is true, then a validity error while parsing will throw a <code>ParsingException</code>.
	 * </p>
	 * 
	 * @param parser
	 *            the SAX2 <code>XMLReader</code> that parses the document
	 ...
	 *            true if the parser should validate the document while parsing
	 * @param factory
	 *            the <code>NodeFactory</code> this builder uses to create objects in the tree
	 * 
	 * @throws XMLException
	 *             if <code>parser</code> does not support the features XOM requires
	 * 
	 ...
	 * 
	 * <p>
	 * Note that relative URLs generally do not work here, as there's no base to resolve them against. This includes relative URLs that point into the file system, though this is somewhat platform dependent. Furthermore, <code>file</code> URLs often only work when they adhere exactly to RFC 2396 syntax. URLs that work in Internet Explorer often fail when used in Java. If you're reading XML from a file, more reliable results are obtained by using the <code>build</code> method that takes a <code>java.io.File</code> object as an argument.
	 * </p>
	 * 
	 * @param systemID
	 *            an absolute URL from which the document is read. The URL's scheme must be one supported by the Java VM.
	 * 
	 * @return the parsed <code>Document</code>
	 * 
	 * @throws ValidityException
	 *             if a validity error is detected. This is only thrown if the builder has been instructed to validate.
	 ...
	 * 
	 * @param in
	 *            the input stream from which the document is read
	 * 
	 * @return the parsed <code>Document</code>
	 * 
	 * @throws ValidityException
	 *             if a validity error is detected; only thrown if the builder has been instructed to validate
	 * @throws ParsingException
	 *             if a well-formedness error is detected
	 * @throws IOException
	 *             if an I/O error such as a broken socket prevents the document from being fully read
	 * @throws NullPointerException
	 *             if <code>in</code> is null
	 */
	 ...
	 *            the input stream from which the document is read
	 * @param baseURI
	 *            an absolute URI for this document; may be null
	 * 
	 * @return the parsed <code>Document</code>
	 * 
	 * @throws ValidityException
	 ...
	 * 
	 * @param in
	 *            the file from which the document is read
	 * 
	 * @return the parsed <code>Document</code>
	 * 
	 * @throws ValidityException
	 ...
	 *            the reader from which the document is read
	 * @param baseURI
	 *            the base URI for this document; may be null
	 * 
	 * @return the parsed <code>Document</code>
	 * 
	 * @throws ValidityException
	 ...
	 * 
	 * @param document
	 *            the string that contains the XML document
	 * @param baseURI
	 *            the base URI for this document; may be null
	 * 
	 * @return the parsed <code>Document</code>
	 ...
	 * 
	 * @param in
	 *            the input source from which the document is read
	 * 
	 * @return the parsed <code>Document</code>
	 * 
	 * @throws ValidityException
	 ...
	 *             if a well-formedness error is detected
	 * @throws IOException
	 *             if an I/O error such as a bad disk prevents the document from being read
	 */
	...
	/**
	 * <p>
	 * Returns this builder's <code>NodeFactory</code>. It returns null if a factory was not supplied when the builder was created.
	 * </p>
	 * 
