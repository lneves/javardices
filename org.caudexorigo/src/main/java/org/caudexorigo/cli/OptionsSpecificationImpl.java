...
class OptionsSpecificationImpl<O> implements OptionsSpecification<O> {
  private final Map<String, OptionSpecificationImpl> m_optionsShortName =
      new HashMap<String, OptionSpecificationImpl>();
  private final Map<String, OptionSpecificationImpl> m_optionsLongName =
      new TreeMap<String, OptionSpecificationImpl>();
  private final Map<Method, OptionSpecificationImpl> m_optionsMethod =
      new HashMap<Method, OptionSpecificationImpl>();
  private final Map<Method, OptionSpecificationImpl> m_optionalOptionsMethod =
      new HashMap<Method, OptionSpecificationImpl>();
  ...
  public OptionsSpecificationImpl(final Class<O> klass) {
    ...
    for (final Method method : methods) {
      if (!Void.class.equals(method.getReturnType())) {
        if (method.isAnnotationPresent(Option.class)) {
          final OptionSpecificationImpl optionSpecification = new OptionSpecificationImpl(method,
              klass);
          ...
          if (optionSpecification.isOptional()) {
            m_optionalOptionsMethod.put(optionSpecification.getOptionalityMethod(),
                optionSpecification);
          }
    ...
    m_cliSpecification = new CliSpecificationImpl(klass.getAnnotation(CommandLineInterface.class),
        m_unparsed, !getMandatoryOptions().isEmpty());
  }
