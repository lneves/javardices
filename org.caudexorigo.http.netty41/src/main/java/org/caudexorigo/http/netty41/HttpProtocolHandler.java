...
public class HttpProtocolHandler extends ChannelInboundHandlerAdapter {
  ...
  public HttpProtocolHandler(RequestRouter requestMapper) {
    this(requestMapper, new DefaultObserver(), new StandardResponseFormatter(false));
  }
    ...
    try {
      if (request.decoderResult().isFailure()) {
        if (request.decoderResult().cause() != null) {
          throw new WebException(request.decoderResult().cause(), HttpResponseStatus.BAD_REQUEST
              .code());
        } else {
          throw new WebException(new IllegalArgumentException(), HttpResponseStatus.BAD_REQUEST
              .code());
        }
      }
      ...
      if (action != null) {
        ...
        throw new WebException(new FileNotFoundException("No available HttpAction for the request"),
            HttpResponseStatus.NOT_FOUND.code());
      }
    } catch (Throwable t) {
      ByteBuf ebuf = ctx.alloc().buffer();
      FullHttpResponse eresponse = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1,
          HttpResponseStatus.INTERNAL_SERVER_ERROR, ebuf);
      ...
      if (w instanceof WebException) {
        ...
        errorAction = new ErrorAction(new WebException(r, HttpResponseStatus.INTERNAL_SERVER_ERROR
            .code()), _rspFmt);
      }
