...
/**
 * <p>
 * Indicates an attempt to assign a name that is not a legal XML name. For example, this might be a name that begins with a digit such as <code>7pins</code> or a name that contains an asterisk such as <code>pt*</code>. In some contexts, this also includes names that are not legal non-colonized names as defined by <cite>Namespaces in XML</cite>.
 * </p>
 * 
...
public class IllegalNameException extends IllegalDataException
{
	...
	/**
	 * <p>
	 * Creates a new <code>IllegalNameException</code> with a detail message and an underlying root cause.
	 * </p>
	 * 
	 * @param message
	 *            a string indicating the specific problem
