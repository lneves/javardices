...
public class HttpProtocolHandler extends SimpleChannelUpstreamHandler {
  ...
  // private Map<String, WebSocketHandler> _webSocketHandlers = new
  // ConcurrentHashMap<String, WebSocketHandler>();
  ...
  public HttpProtocolHandler(RequestRouter requestMapper, RequestObserver customObserver,
      ResponseFormatter customResponseFormtter) {
    _requestMapper = requestMapper;
  ...
  public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
    ...
    if (msg instanceof HttpRequest) {
      ...
      if (is100ContinueExpected(req)) {
        ...
        ctx.setAttachment(new DefaultHttpRequest(HttpVersion.HTTP_1_1, req.getMethod(), req
            .getUri()));
      }
      ...
      throw new IllegalArgumentException(String.format(
          "Invalid Object type received by HttpHandler: '%s'", msg.getClass().getCanonicalName()));
    }
  ...
  private void handleHttpRequest(ChannelHandlerContext ctx, HttpRequest request) {
    // if (_hasWebSocketSupport && _webSocketHandlers.containsKey(request.getUri()) &&
    // HttpHeaders.Values.UPGRADE.equalsIgnoreCase(request.getHeader(HttpHeaders.Names.CONNECTION))
    // &&
    ...
    // if (log.isDebugEnabled())
    // {
    // log.debug("Received WebSocket upgrade request. Origin: '{}'; Location: '{}'",
    // origin, location);
    // }
    //
    ...
    // HttpResponse response = new DefaultHttpResponse(HttpVersion.HTTP_1_1, new
    ...
    //
    // if (request.containsHeader(HttpHeaders.Names.SEC_WEBSOCKET_KEY1) &&
    // request.containsHeader(HttpHeaders.Names.SEC_WEBSOCKET_KEY2))
    // {
    // // New handshake method with a challenge:
    // response.addHeader(HttpHeaders.Names.SEC_WEBSOCKET_ORIGIN,
    // request.getHeader(HttpHeaders.Names.ORIGIN));
    // response.addHeader(HttpHeaders.Names.SEC_WEBSOCKET_LOCATION,
    ...
    // pipeline.replace("http-encoder", "websocket-encoder", new
    // WebSocket13FrameEncoder(false));
    // pipeline.replace("http-decoder", "websocket-decoder", new
    // WebSocket13FrameDecoder(false, false));
    //
    ...
    try {
      ...
      if (t instanceof WebException) {
        ...
        errorAction = new ErrorAction(new WebException(t, HttpResponseStatus.INTERNAL_SERVER_ERROR
            .getCode()), _rspFmt);
      }
