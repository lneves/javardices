...
public class JptConditionalNode extends JptParentNode {
  ...
  JptConditionalNode(String jpt_exp, boolean isInSlot) {
    _isInSlot = isInSlot;
    String boolExp = StringUtils.isBlank(jpt_exp) ? "true"
        : StringUtils.replace(jpt_exp, "\'", "\"");
  ...
  public void render(Map<String, Object> context, Writer out) throws IOException {
    try {
      ...
      // System.out.printf("JptConditionalNode.render -> expression: '%s'; condition:
      // %s%n", _bool_exp, condition);
      ...
      String emsg = String.format(
          "Error processing JptConditionalNode:%nexpression: '%s';%ncontext: %s;%nmessage: '%s'",
