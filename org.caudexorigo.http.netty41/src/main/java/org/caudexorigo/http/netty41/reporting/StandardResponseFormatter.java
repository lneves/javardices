  ...
  public void formatResponse(ChannelHandlerContext ctx, FullHttpRequest request,
    ...
    if (MessageBody.allow(response.status().code())) {
      try {
        ...
        String html = String.format(ErrorTemplates.getTemplate(response.status().code()), response
            .status().code(), response.status().reasonPhrase(), request.method().toString(),
            getStackTrace(root, showFullErrorInfo));
