...
/**
 * These methods are not fully general. You would need to uncomment some lines to make
 * this a public API. Certain preconditions for these methods to operate correctly are
 * true in the context of XOM, but may well not be true in a more general context.
 * 
...
class URIUtil {
  ...
  static boolean isOpaque(String uri) {
    ...
    if (!Verifier.isAlpha(uri.charAt(0)))
      return false;
    /*
     * for (int i = 1; i < colon; i++) { if (!Verifier.isSchemeCharacter(uri.charAt(i))) {
     * return false; } }
     */
    return true;
  ...
  static boolean isAbsolute(String uri) {
    ...
    // legal URI. Thus we don't have to check everything here.
    /*
     * if (!Verifier.isAlpha(uri.charAt(0))) return false; for (int i = 1; i < colon; i++)
  ...
  static String absolutize(String baseURI, String spec) {
    ...
    /*
     * if (R.scheme != null) { T.scheme = R.scheme; T.authority = R.authority; T.query =
     * R.query; T.path = removeDotSegments(R.path); } else {
     */
    if (R.authority != null) {
  ...
  static String removeDotSegments(String path) {
    ...
    while (path.length() > 0) {
      ...
      /*
       * else if (path.equals(".") || path.equals("..")) { path = ""; } else if
       * (path.startsWith("../")) { path = path.substring(3); } else if
