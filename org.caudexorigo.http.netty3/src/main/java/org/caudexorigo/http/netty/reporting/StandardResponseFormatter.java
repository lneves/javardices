...
public class StandardResponseFormatter implements ResponseFormatter {
   ...
   * 
   * @see
   * org.caudexorigo.http.netty.reporting.ResponseFormtter#formatResponse(org.jboss.netty.
   * handler.codec.http.HttpRequest, org.jboss.netty.handler.codec.http.HttpResponse)
   */
  @Override
  public void formatResponse(ChannelHandlerContext ctx, HttpRequest request,
  ...
  public void formatResponse(ChannelHandlerContext ctx, HttpRequest request, HttpResponse response,
    ...
    if (MessageBody.allow(rsp_code)) {
      try {
        String html = String.format(ErrorTemplates.getTemplate(response.getStatus().getCode()),
            response.getStatus().getCode(), response.getStatus().getReasonPhrase(), request
                .getMethod().toString(), getStackTrace(error, showFullErrorInfo));
