...
public class DefaultRouter implements RequestRouter
{
	...
	private final CacheKeyBuilder cacheKeyBuilder = new CacheKeyBuilder()
	{
		...
		{
			String path = StringUtils.substringBefore(request.uri(), "?");

			String accept_enconding = StringUtils.trimToEmpty(request.headers().get(HttpHeaderNames.ACCEPT_ENCODING));
	...
	public HttpAction map(ChannelHandlerContext ctx, FullHttpRequest req)
	{
		...
		else if ("/gz_hello".equals(req.uri()))
		{
			return gz_hello;
		}
		else if ("/gz_cached_hello".equals(req.uri()))
		{
			return gz_cached_hello;
		}
		else if ("/cached_hello".equals(req.uri()))
		{
			return cached_hello;
		}
		return null;
