...
/**
 * <p>
 * This class is responsible for creating XOM <code>Document</code> objects from a URL,
 * file, string, or input stream by reading an XML document. A SAX parser is used to read
 * the document and report any well-formedness errors.
 * </p>
 * 
...
public class Builder {
  ...
  /**
   * <p>
   * Creates a <code>Builder</code> that uses the default node factory and chooses among
   * any available SAX2 parsers. In order of preference, it looks for:
   * </p>
   * 
   * <ol>
   ...
   * <li>The platform default specified by the <code>org.xml.sax.driver</code> system
   * property</li>
   * </ol>
   * 
   * <p>
   ...
   * http://xml.org/sax/features/external-parameter-entities features XOM requires.
   * Parsers that don't are rejected automatically.
   * </p>
   * 
  ...
  /**
   * <p>
   * Creates a <code>Builder</code> based on an optionally validating parser. If the
   * <code>validate</code> argument is true, then a validity error while parsing will
   * cause a fatal error; that is, it will throw a <code>ValidityException</code>.
   * </p>
   * 
   * @param validate true if the parser should validate the document while parsing
   *
   * @throws XMLException if no satisfactory parser is installed in the local class path
   */
  ...
  /**
   * <p>
   * Creates a <code>Builder</code> based on an optionally validating parser that builds
   * node objects with the supplied factory. If the <code>validate</code> argument is
   * true, then a validity error while parsing will cause a fatal error; that is, it will
   * throw a <code>ValidityException</code>.
   * </p>
   * 
   * @param validate true if the parser should validate the document while parsing
   * @param factory the <code>NodeFactory</code> that creates the node objects for this
   *        <code>Builder</code>
   *
   * @throws XMLException if no satisfactory parser is installed in the local class path
   */
  ...
  private static String[] parsers = {
      "org.caudexorigo.nu.xom.XML1_0Parser",
      "org.caudexorigo.nu.xom.JDK15XML1_0Parser",
  ...
  private static void setupParser(XMLReader parser, boolean validate)
    ...
    if (parserName.equals("org.caudexorigo.nu.xom.XML1_0Parser")
        || parserName.equals("org.caudexorigo.nu.xom.JDK15XML1_0Parser")
        ...
        || parserName.equals("com.sun.org.apache.xerces.internal.jaxp.SAXParserImpl$JAXPSAXParser")) // JDK
                                                                                                     // 1.6
    {
      try {
        parser.setFeature(
  ...
  /**
   * <p>
   * Creates a <code>Builder</code> that uses the specified SAX <code>XMLReader</code>.
   * Custom SAX features and properties such as schema validation can be set on this
   * <code>XMLReader</code> before passing it to this method.
   * </p>
   * 
   * @param parser the SAX2 <code>XMLReader</code> that parses the document
   * 
   * @throws XMLException if <code>parser</code> does not support the features XOM
   *         requires
   */
  ...
  /**
   * <p>
   * Creates a <code>Builder</code> that uses the specified <code>NodeFactory</code> to
   * create node objects.
   * </p>
   * 
   * @param factory the <code>NodeFactory</code> that creates the node objects for this
  ...
  /**
   * <p>
   * Creates a optionally validating <code>Builder</code> based on the specified parser
   * object. Custom SAX features and properties such as schema validation can be set on
   * this <code>XMLReader</code> before passing it to this method.
   * </p>
   * 
   * <p>
   * If the validate argument is true, then a validity error while parsing will cause a
   * fatal error; that is, it will throw a <code>ParsingException</code>
   * </p>
   * 
   * @param parser the SAX2 <code>XMLReader</code> that parses the document
   * @param validate true if the parser should validate the document while parsing
  ...
  /**
   * <p>
   * Creates an optionally validating <code>Builder</code> that reads data from the
   * specified parser object and constructs new nodes using the specified factory object.
   * Custom SAX features and properties such as schema validation can be set on this
   * <code>XMLReader</code> before passing it to this method.
   * </p>
   * 
   * <p>
   * If the <code>validate</code> argument is true, then a validity error while parsing
   * will throw a <code>ParsingException</code>.
   * </p>
   * 
   * @param parser the SAX2 <code>XMLReader</code> that parses the document
   * @param validate true if the parser should validate the document while parsing
   * @param factory the <code>NodeFactory</code> this builder uses to create objects in
   *        the tree
   * 
   * @throws XMLException if <code>parser</code> does not support the features XOM
   *         requires
  ...
  private static boolean knownGoodParser(XMLReader parser) {
    ...
    for (int i = 0; i < parsers.length; i++) {
      if (parserName.equals(parsers[i]))
        return true;
    }
    return false;
  ...
  private void setHandlers() {
    ...
    XOMHandler handler;
    if ((factory == null
        || factory.getClass().getName().equals("org.caudexorigo.nu.xom.NodeFactory"))
   ...
   * 
   * <p>
   * Note that relative URLs generally do not work here, as there's no base to resolve
   * them against. This includes relative URLs that point into the file system, though
   * this is somewhat platform dependent. Furthermore, <code>file</code> URLs often only
   * work when they adhere exactly to RFC 2396 syntax. URLs that work in Internet Explorer
   * often fail when used in Java. If you're reading XML from a file, more reliable
   * results are obtained by using the <code>build</code> method that takes a
   * <code>java.io.File</code> object as an argument.
   * </p>
   * 
   * @param systemID an absolute URL from which the document is read. The URL's scheme
   *        must be one supported by the Java VM.
   * 
   * @return the parsed <code>Document</code>
   * 
   * @throws ValidityException if a validity error is detected. This is only thrown if the
   *         builder has been instructed to validate.
   * @throws ParsingException if a well-formedness error is detected
   * @throws IOException if an I/O error such as a broken socket prevents the document
   *         from being fully read
   */
   ...
   * @return the parsed <code>Document</code>
   * 
   * @throws ValidityException if a validity error is detected; only thrown if the builder
   *         has been instructed to validate
   * @throws ParsingException if a well-formedness error is detected
   * @throws IOException if an I/O error such as a broken socket prevents the document
   *         from being fully read
  ...
  /**
   * <p>
   * Reads the document from an input stream while specifying a base URI (which need not
   * be the stream's actual URI).
   * </p>
   * 
   * @param in the input stream from which the document is read
  ...
  /**
   * <p>
   * Reads the document from a file. The base URI of the document is set to the location
   * of the file.
   * </p>
   * 
   ...
   *         builder has been instructed to validate.
   * @throws ParsingException if a well-formedness error is detected
   * @throws IOException if an I/O error such as a bad disk prevents the file from being
   *         read
   */
   ...
   *         builder has been instructed to validate.
   * @throws ParsingException if a well-formedness error is detected
   * @throws IOException if an I/O error such as a bad disk prevents the document from
   *         being fully read
   */
   ...
   *         builder has been instructed to validate.
   * @throws ParsingException if a well-formedness error is detected
   * @throws IOException if an I/O error such as a bad disk prevents the document's
   *         external DTD subset from being read
   */
  ...
  private String canonicalizeURL(String uri) {
    ...
    try {
      ...
      if (authority != null)
        canonicalForm.append(authority);
      if (port >= 0)
        canonicalForm.append(":" + port);
      canonicalForm.append(path);
      if (query != null)
        canonicalForm.append("?" + query);
  ...
  private Document build(InputSource in)
    ...
    if (errorHandler instanceof ValidityRequired) {
      ValidityRequired validityHandler = (ValidityRequired) errorHandler;
  ...
  /**
   * <p>
   * Returns this builder's <code>NodeFactory</code>. It returns null if a factory was not
   * supplied when the builder was created.
   * </p>
   * 
