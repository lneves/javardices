...
class ArgumentTyperImpl<O> implements ArgumentTyper<O> {
   ...
   * @inheritdoc
   */
  public TypedArguments typeArguments(final ValidatedArguments validatedArguments)
  ...
  private void typeUnparsedArguments(final ValidatedArguments validatedArguments,
      final TypedArgumentsImpl typedArguments) {
    if (m_specification.hasUnparsedSpecification()) {
      ...
      typedArguments.setUnparsedValue(getValue(specification.getMethod(), validatedArguments
          .getUnparsed(), specification));
    }
  }
  ...
  private TypedArgumentsImpl typeParsedArguments(final ValidatedArguments validatedArguments) {
    ...
    for (final OptionSpecification optionSpecification : m_specification) {
      if (validatedArguments.containsAny(optionSpecification.getAllNames()) || optionSpecification
          .hasDefaultValue()) {
        final Object value;
        if (optionSpecification.hasValue()) {
          value = getValue(validatedArguments, optionSpecification.getMethod(),
              optionSpecification);
        } else {
          value = Boolean.TRUE;
        }
  ...
  private Object getValue(final ValidatedArguments arguments, final Method method,
      final OptionSpecification specification) {
    final List<String> values = arguments.containsAny(specification.getAllNames()) ? arguments
        .getValues(specification.getAllNames()) : specification.getDefaultValue();
    return getValue(method, values, specification);
  }

  @SuppressWarnings("unchecked")
  private Object getValue(final Method method, final List<String> values,
    ...
    try {
        ...
        if (type.isEnum()) {
          ...
          if (type.equals(Character.TYPE) || type.equals(Character.class)) {
            ...
            throw new UnsupportedOperationException(String.format(
                "Method (%s) return type not supported for reading argument values", method
                    .toGenericString()));
          }
        } else if (type.equals(Character.class)) {
          // there is always an exception, and java.lang.Character can't constructed from
          // a string
          if (value.length() == 1) {
            result.add(value.charAt(0));
          } else {
      ...
      m_validationErrorBuilder.invalidValueForType(specification, unsupportedNumberFormatMessage(
          e));
