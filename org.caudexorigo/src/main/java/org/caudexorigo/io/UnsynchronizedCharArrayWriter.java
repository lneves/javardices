...
public class UnsynchronizedCharArrayWriter extends Writer {
  ...
  public void write(char c[], int off, int len) {
    if ((off < 0) || (off > c.length) || (len < 0) || ((off + len) > c.length) || ((off
        + len) < 0)) {
  ...
  /**
   * Resets the buffer so that you can use it again without throwing away the already
   * allocated buffer.
   */
  public void reset() {
    count = 0;
   ...
   * still be required. Note: Invoking this method in this class will have no effect.
   */
  public void close() {}
