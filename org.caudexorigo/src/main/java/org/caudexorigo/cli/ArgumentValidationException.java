...
public class ArgumentValidationException extends CliException {
  ...
  private static final ResourceBundle m_messages = ResourceBundle.getBundle(
      "org.caudexorigo.cli.Messages", Locale.getDefault());
  ...
  public interface ValidationError {
    enum ErrorType {
      ...
      MisplacedOption {
        public String getDescription(final ValidationError error) {
          return String.format(m_messages.getString("validationError.MisplacedOption"), error
              .getMessage());
        }
      },
      UnexpectedValue {
      ...
      InvalidValueForType {
        public String getDescription(final ValidationError error) {
          return String.format(m_messages.getString("validationError.InvalidValueForType"), error
              .getMessage());
        }
      },
      UnableToConstructType {
        public String getDescription(final ValidationError error) {
          return String.format(m_messages.getString("validationError.UnableToConstructType"), error
              .getMessage());
        }
      },
      PatternMismatch {
        public String getDescription(final ValidationError error) {
          return String.format(m_messages.getString("validationError.PatternMismatch"), error
              .getMessage());
        }
      },
      HelpRequested {
  ...
  private static class ValidationErrorImpl implements ValidationError {
    ...
    public ValidationErrorImpl(final ErrorType errorType,
        final ArgumentSpecification specification) {
    ...
    public String toString() {
      return String.format("%s: %s", getErrorType().getDescription(ValidationErrorImpl.this),
          getSpecification());
    }
  ...
  static ValidationError createUnexpectedOptionError(final String name) {
    return new ValidationErrorImpl(ErrorType.UnexpectedOption, new UnexpectedOptionSpecification(
        name));
  }

  static ValidationError createAdditionalValuesError(
      final ArgumentSpecification optionSpecification) {
  ...
  static ValidationError createUnexpectedValueError(
      final ArgumentSpecification optionSpecification) {
  ...
  static ValidationError createInvalidValueForType(final ArgumentSpecification optionSpecification,
      final String message) {
  ...
  static ValidationError createUnableToConstructType(
      final ArgumentSpecification optionSpecification, final String message) {
    return new ValidationErrorImpl(ErrorType.UnableToConstructType, optionSpecification, message);
  }

  static ValidationError createPatternMismatch(final ArgumentSpecification optionSpecification,
      final String message) {
